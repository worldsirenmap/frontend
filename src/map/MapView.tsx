...
import Map, {Layer, MapLayerMouseEvent, MapRef, NavigationControl, Popup, Source, VectorTileSource, ViewStateChangeEvent} from 'react-map-gl/maplibre';
...
import {LocalMarkerState, PopupAction, PopupContext, PopupState} from "../types.d.ts";
...
export default () => {
    ...
    const mapOnClick = useCallback((e: MapLayerMouseEvent) => {
        if (e.features && e.features[0] && e.features[0].source == 'sirenmap') {
            navigate('site', e.features[0].id as string)
        }
    }, [navigate])
    ...
    return (
        <Map
            ...
            mapStyle={'https://worldsirenmap.net/map/style/wsm.json'}
            cursor={cursor}
